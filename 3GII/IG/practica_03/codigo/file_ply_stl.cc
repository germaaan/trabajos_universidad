...
void read( const char * nombreArchivo, vector<float> &vertices, vector<int> &faces )
{
   ...
   char 
      buffer[unsigned(bufferCapacity)];
   string 
      token ;
   ...
   if ( ! src.is_open() ) 
   {
      string msg = string("Cannot open source file '") + sfn + "' for reading" ; 
      error(msg.c_str());
   }
   ...
   if ( token != "ply" )
      error("input file does not begins with 'ply'");

   src.getline(buffer,bufferCapacity);

   cout << "reading ply file '" + sfn + "'" << endl ;
   ...
   {
      if ( src.eof() ) 
         error("premature end of file in header");

     src >> token ;

     if ( token == "end_header" )
     {  if ( state != 2 )
           error("no vertex or face element found in header");
        src.getline(buffer,bufferCapacity);
        inHeader = false ;
     }
     ...
     {  src >> token ;
        if ( token != "ascii" )
        {  string msg = string("cannot read ply file in non-ascii format, format is: '")+token+"'" ;
           error(msg.c_str());
        }
        src.getline(buffer,bufferCapacity);
     }
   ...
   if ( nVertexs == 0 || nFaces == 0 )
      error("no vertexs or faces found in source file");
   ...
   {
      if ( src.eof() ) 
         error("premature end of file in vertex list");
   ...
   {
      if ( src.eof() ) 
         error("premature end of file in face list");
      ...
      if ( nv != 3 )
         error("cannot read PLY files with non-triangular faces. Sorry!");
