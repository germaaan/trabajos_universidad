...
public class Munchkin {
    ...
    private LinkedList<Tesoro> mazoTesoros = new LinkedList<Tesoro>();
    private LinkedList<Tesoro> descarteTesoros = new LinkedList<Tesoro>();
    private Monstruo monstruoEnJuego;
    private LinkedList<Monstruo> mazoMonstruos = new LinkedList<Monstruo>();
    private LinkedList<Monstruo> descarteMonstruos = new LinkedList<Monstruo>();
    ...
    private void primero() {
        this.indexJugador = (int) Math.floor(Math.random() * this.jugadores.size());
    ...
    // Visibilidad cambiada para poder implementar acción del boton "Robar"
    ...
    public void siguienteTurno() throws Exception {
        ...
        if (this.jugadorActivo != null) {
            /*
             * Hemos tenido que cambiar la implementacion del metodo
             * terminarTurno() porque imposibilitada continuar el juego.
             */
            fin = this.jugadorActivo.terminarTurno();
        }

        if ((this.jugadorActivo != null) && (!fin)) {
            Exception e = new Exception("El jugador tiene más de 4 tesoros ocultos,"
                    + " no se puede pasar al siguiente turno.");
            throw e;
        }
    ...
    public LinkedList<Tesoro> equiparseJugador(LinkedList<Tesoro> listaTesoros) {
        LinkedList<Tesoro> tesorosNoEquipados = this.jugadorActivo.equiparse(listaTesoros);
    ...
    public int desarrollarCombate() {
        ...
        int dado;
        /*
         * // Como sería según diagrama 
         * if (this.monstruoEnJuego.obtenerNivel()
         * < this.jugadorActivo.nivelDeCombate()) { tesorosGanados =
         ...
         *
         * if (this.jugadorActivo.obtenerNivel() == 10) { resultado = 10; } else
         * { resultado = 1; } } else if (this.monstruoEnJuego.obtenerNivel() >=
         * this.jugadorActivo.nivelDeCombate()) { dado = (int)
         * Math.floor(Math.random() * (6 + 1));
         *
         * if (dado < 5) {
         * this.jugadorActivo.modificarNivel(this.monstruoEnJuego.obtenerNivelesPerdidos());
         * resultado = -1; } else { resultado = 0; } }
         */
        ...
        if (this.monstruoEnJuego.obtenerNivel() < this.jugadorActivo.nivelDeCombate()) {
            ...
            if (dado < 5) {
                ...
                if (this.jugadorActivo.obtenerNivel() <= 0) {
                    this.muerteJugador();
                }
    ...
    public void muerteJugador() {
        ...
        LinkedList<Tesoro> tesorosPerdidos = this.jugadorActivo.muerte();
    ...
    public void comprarNivelesJugador(LinkedList<Tesoro> listaTesoros) {
        Tesoro tesoro;
        int contNiveles = 0;
        int contOro = 0;
