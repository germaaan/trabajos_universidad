...
Matriz2D_1::Matriz2D_1(const unsigned int &fils, const unsigned int &cols) {
...
Matriz2D_1::Matriz2D_1(const unsigned int &fils, const unsigned int &cols, const unsigned int &val) {
    this->PideMemoria(fils, cols);
...
void Matriz2D_1::PonValor(const unsigned int &fil, const unsigned int &col, const unsigned int &val) {
    if ((fil < this->filas) && (col < this->columnas))
        this->datos[fil][col] = val;
}

int Matriz2D_1::LeeValor(const unsigned int &fil, const unsigned int &col) {
    if ((fil < this->filas) && (col < this->columnas))
...
int& Matriz2D_1::operator()(int fil, int col) {
...
int Matriz2D_1::operator()(int fil, int col) const {
    if ((fil < 0) || (fil > this->filas) || (col < 0) || (col > this->columnas)) {
        cerr << "Error: indice fuera del rango de casillas accesible." << endl;
        exit(1);
    }
...
ostream & operator <<(ostream &out, const Matriz2D_1 &m) {
...
void Matriz2D_1::PideMemoria(const unsigned int &fils, const unsigned int &cols) {
    try {
